class Grid():
    ...
    def register_from_file(self, filepath, corner_coord=None):
        with open(filepath, "r") as f:
            ...
            if corner_coord is None:
                ...
                start_row, start_col, start_layer = corner_coord
            
            for y_i, row in enumerate(cells):
                for x_i, cell in enumerate(row):
                    position = (x_i + start_col, y_i + start_row, start_layer)
                    self.register(Cube(cell, position, parent=self))

    def get_adjoining(self, position):
        x, y, z = position
        positions = [(x_i, y_i, z_i) for x_i in [x-1, x, x+1] for y_i in [y-1, y, y+1] for z_i in [z-1, z, z+1] if (x_i, y_i, z_i) != (x, y, z)]
        cubes = tuple(map(self.cubes.get, positions))
        return cubes

    def advance(self, iterations, report):
        if report:
            self.render()
    ...
    def expand(self):
        ...
        x_bound, y_bound, z_bound = tuple(zip(min_dims, max_dims))
        _ = [self.register(Cube(".", (x_i, y_i, z_i), parent=self)) for x_i in range(*x_bound) for y_i in range(*y_bound) for z_i in range(*z_bound) if (x_i, y_i, z_i) not in self.cubes]

    def get_active_cubes(self):
        return sum(1 for cube in self.cubes.values() if cube.active)

    def render(self):
        max_dims = [max(el)+1 for el in zip(*self.cubes.keys())]
        min_dims = [min(el) for el in zip(*self.cubes.keys())]
        x_bound, y_bound, z_bound = tuple(zip(min_dims, max_dims))
        for z_i in range(*z_bound):
            rows = []
            for y_i in range(*y_bound):
                row = []
                for x_i in range(*x_bound):
                    row.append(str(self.cubes[(x_i, y_i, z_i)]))
